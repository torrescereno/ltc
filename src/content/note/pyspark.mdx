...
Un RDD (Resilient Distributed Dataset) es una estructura de datos distribuida de elementos
que se pueden procesar en paralelo en un clúster.
Los RDDs permiten realizar operaciones de manera distribuida y tolerante a fallos,
lo que es clave para procesar grandes volúmenes de datos.

1- Distribuido: Los RDDs se dividen en particiones que se distribuyen a lo largo de diferentes nodos en un clúster.

2- Inmutable: Una vez creado, un RDD no se puede modificar.
Sin embargo, se pueden crear nuevos RDDs a partir de operaciones en uno existente.

3- Tolerante a fallos: Spark puede reconstruir automáticamente un RDD perdido debido a fallos en el hardware o software,
utilizando el linaje de transformación que llevó a la creación del RDD original.

4- Transformaciones y Acciones: Los RDDs soportan dos tipos de operaciones:
transformaciones (como map, filter) que crean un nuevo RDD a partir de uno existente,
y acciones (como collect, count) que devuelven un resultado al driver programando y ejecutando las transformaciones definidas.

### DataFrame

Un DataFrame es una estructura de datos distribuida de alto nivel que organiza los datos
en un formato tabular similar a una tabla en una base de datos relacional, con columnas y filas.

1- Distribuido:
Los DataFrames se distribuyen en un clúster,
lo que permite procesar grandes volúmenes de datos de manera paralela y eficiente.
Cada DataFrame se divide en particiones que se distribuyen a lo largo de diferentes nodos.

2- Esquematizado:
A diferencia de los RDDs, los DataFrames tienen un esquema asociado,
que define los nombres de las columnas y los tipos de datos.
Esto permite realizar consultas estructuradas y optimizadas sobre los datos utilizando un lenguaje similar a SQL.

3- Optimizado:
Los DataFrames aprovechan el motor de optimización Catalyst de Spark,
que automáticamente optimiza las consultas y transforma los planes de ejecución para maximizar el rendimiento,
minimizando el uso de recursos y mejorando la velocidad.

4- Interoperabilidad:
Los DataFrames pueden ser creados a partir de diferentes fuentes de datos,
como archivos CSV, JSON, Parquet, bases de datos SQL, y más.
Además, son compatibles con el API de SQL y permiten realizar consultas SQL directamente sobre ellos.

5- Transformaciones y Acciones:
Al igual que los RDDs, los DataFrames soportan transformaciones (como select, filter, groupBy)
que devuelven nuevos DataFrames y acciones (como show, count, collect)
que devuelven resultados al driver programando y ejecutando las transformaciones definidas.
